...
- If the resolved target is missing locally, let the helper try CMSIS-Pack auto-install. It first tries direct names, then family-style fuzzy search with pyOCD glob patterns such as `MCX A36x`, `MCXA366`, `MCXA366x`, or `a345x`. If one pack matches, it installs it; if multiple packs match, it reports retry suggestions like `MCXA365` or `MCXA365VLL`.
